...
async function self({ id }) {
  try {
    let user = await User.findById(id, {avatar_url: 1, name: 1, email: 1, _id: 0})
...
async function request({ id }) {
  try {
    let user = await User.findById(id, {_id:0, incoming_request:1, outgoing_request:1})
    return user.toObject()
  } catch (e) {
    console.error(e)
    throw e
